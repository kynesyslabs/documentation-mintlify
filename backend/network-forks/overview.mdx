...
The first production fork is **`osDenomination`**, which activates the [sub-DEM (OS) denomination](#fork-registry). A second fork, **`gasFeeSeparation`** (DEM-665), splits the single lump-sum gas fee into `network`/`rpc`/`additional` components with per-component burn/treasury/rpc-operator distribution.
