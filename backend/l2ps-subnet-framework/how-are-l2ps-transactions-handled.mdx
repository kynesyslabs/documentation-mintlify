...
<Steps>
  <Step title="⚡ Executed (l2ps_pending)" icon="bolt">
    Validated locally, GCR edits generated **(instant)**
  </Step>
  <Step title="📦 Batched (l2ps_batched)" icon="box">
    Included in an L1 batch transaction **(per block)**
  </Step>
  <Step title="✓ Confirmed (l2ps_confirmed)" icon="check">
...
|-------------------|----------------------------------------|-------------|
| `l2ps_pending`    | Validated locally, GCR edits generated | Yes (if batch fails) |
| `l2ps_batched`    | Included in an L1 batch transaction    | Yes (if block fails) |
| `l2ps_confirmed`  | L1 block finalized                     | No |
