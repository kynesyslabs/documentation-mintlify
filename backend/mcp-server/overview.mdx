...
The MCP server is OFF by default and must be enabled by setting `MCP_ENABLED=true`. Once enabled, it is started when you run your Demos Network node (`src/index.ts`). The integration follows these steps:
