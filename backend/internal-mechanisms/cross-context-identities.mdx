...
Each context has its own payload shape and validation rules. The full list of GCR routines lives at `src/libs/blockchain/gcr/gcr_routines/`. (TLSNotary attestations are tracked separately in the dedicated `gcr_tlsnotary` table rather than in the `identities` object.)
