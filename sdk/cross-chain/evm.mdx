---
title: "Evm"
description: "The Ethereum Virtual Machine (EVM) is the runtime environment for smart contracts in Ethereum. It's a fundamental part of the Ethereum network and is ..."
icon: "ethereum"
---
...
The EVM SDK supports multiple EVM-compatible networks:

| Network | Mainnet | Testnet | Chain ID |
|---------|---------|---------|----------|
...
For testing purposes, you can use faucets to get testnet tokens:
...
The SDK provides methods to interact with smart contracts. Note that `abi` must be passed as a **JSON string**, not a JavaScript array — pass it through `JSON.stringify(abi)` if you have it as an array.
...
// Write to contract — returns the signed-and-broadcast tx hash as a string
const txHash = await instance.writeToContract(
    contract,
    "functionName",
    [arg1, arg2],
    { gasLimit: 200_000, value: "0.01" }, // optional; value is in ETH (parsed via parseEther)
)
```
...
```ts
// Listen for a specific event (contract, abi, event, timeout?)
const eventData = await instance.listenForEvent(contractAddress, abi, "EventName", 5000)

// Listen for all events (returns a function to remove the listener)
const unsubscribe = instance.listenForAllEvents(contractAddress, abi, (...args) => {
