...
- **Flexible access control**: Three modes (`owner`, `public`, `restricted`) cover private, collaborative, and public scenarios, plus allowlists, blacklists, and groups. See the [Access Control guide](./access-control) for details.
...
- **Predictable limits**: 1MB payload per program, 64 levels of nesting.
...
const address = StorageProgram.deriveStorageAddress(
  ...
  nonce,           // Sender's account nonce (required)
...
2. **WRITE**: Replace the entire `data` field (does not merge; use granular ops for partial updates)
...
Each program chooses one of three modes (`owner`, `public`, `restricted`), optionally combined with allowlists, blacklists, and groups. Rather than duplicating the full matrix here, see the [Access Control guide](./access-control) for the authoritative table, example flows, and security checklists.
...
//   MAX_SIZE_BYTES: 1048576,        // 1MB total
//   MAX_JSON_NESTING_DEPTH: 64,     // 64 levels of nested objects
//   PRICING_CHUNK_BYTES: 10240,     // 10KB fee chunk
//   FEE_PER_CHUNK: OS_PER_DEM,      // 1 DEM per chunk
// }
```
...
- Exceed 1MB limit (transaction will fail)
