...
`txHash` is added by the SDK after confirm/broadcast; node responses may omit it. Only hashes (`responseHash`, `responseHeadersHash`) are stored on-chain; not the full response.
...
console.log(res.responseHash, res.responseHeadersHash)
