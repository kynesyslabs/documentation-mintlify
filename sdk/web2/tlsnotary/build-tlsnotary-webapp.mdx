...
```typescript
// Connect via wallet extension (instead of mnemonic)
await demosInstance.connectWalletExtension();

// Use WithConfirmation methods for wallet extension apps
const { tlsn, tokenId } = await tlsnService.createTLSNotaryWithConfirmation(
  { targetUrl },
  {
    onConfirm: async (details) => {
      // In production: wallet extension shows approval UI
      // details.transaction contains the unsigned tx
      // details.amount shows fee (1 DEM for attestation)
      return await walletExtension.requestApproval(details.transaction);
    }
  }
);

// Perform attestation (no blockchain transaction needed)
const result = await tlsn.attest({ url: targetUrl });

// Store proof with user confirmation
await tlsnService.storeProofWithConfirmation(
  tokenId,
  JSON.stringify(result.presentation),
  { storage: 'onchain' },
  {
    onConfirm: async (details) => {
      // details.amount shows storage fee (1 + size_kb DEM)
      return await walletExtension.requestApproval(details.transaction);
    }
  }
);
