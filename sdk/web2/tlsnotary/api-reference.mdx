...
```typescript
// Returns a bigint in OS (smallest units): 6 DEM for a 5KB proof (1 base + 1 per KB).
// Convert for display with denomination.osToDem(fee).
const fee = service.calculateStorageFee(5);
```
...
interface AttestationTokenResponse {
    ...
    txHash: string;        // Transaction hash of the attestation request
}
```
...
interface StoreProofResponse {
    ...
    storageFee: bigint;      // Total fee burned, as a bigint in OS (smallest units)
...
interface TransactionDetails {
    ...
    txHash: string;        // Transaction hash
    amount: bigint;        // Amount being burned, as a bigint in OS (smallest units)
...
The `commit` ranges and `onStatus` callback are passed as separate positional arguments, not as fields of the `request` object.
...
interface AttestResult {
    ...
    verification: VerificationResult;   // Verification result (time, serverName, sent, recv, notaryKey, verifyingKey)
}
```
